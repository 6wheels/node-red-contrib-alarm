...
module.exports = function (RED) {
  function AnamicoAlarmChangeState(config) {
    ...
    node.on("input", function (msg) {
      ...
      if (this.configError) {
        return;
      } // ignore everything if in error state, can only redeploy to fix this state
      ...
      if (node.format == Formats.value) {
        // raw value mode, the incoming value is the desired alarm state
        if (isNaN(msg.payload)) {
          node.error("non-numeric payload received", msg);
          node.status({
            fill: "red",
            shape: "dot",
            text: "non-numeric payload received",
          });
          return;
        }
        msg.payload = {
      ...
      node._panel &&
        node._panel.setState(msg, function (result) {
          node.status({
            fill: result.error || node._panel.isAlarm ? "red" : "green",
            shape: "dot",
            text: result.label,
          });
        });
     ...
     */
    node.panel &&
      node._panel.registerStateListener(this, function (msg) {
        if (node.configError) {
